...
valuepoints = px.scatter(logindustries, x="Market Capitalization size", y="Overall points", color=' Country', 
...
def industrymetric(industry):
    ...
    overall = collection.find({' Country': industry })
    df = pd.DataFrame(overall)
    df = df.astype({" Country": str})
    df = df[['Overall points', 'Market Capitalization size','Name', 'Ticker', ' Country', 'Dividend points normal', 'Revenues points normal', 'Free Cash Flow points normal', 'Net Income points normal',
...
industrylist = allindustries[' Country'].tolist()
#st.write(industrylist)

options = st.selectbox(
     'Pick an country you want to see stats for',
     industrylist#,
#     #industrylist[69]
)
